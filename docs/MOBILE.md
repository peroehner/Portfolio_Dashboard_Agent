...
| **Symbol** (stack) | `GET /symbols/{symbol}/inspector` | Summary: SAI chips = Action/`!` · Conf · Score · Fit Band · Sentiment, then offset Portfolio Intent (**role glyph + label**, e.g. shield / Core + opportunistic); Drivers; wide paired rows **Holdings ‖ Thresholds** and **Key Fundamentals ‖ Analyst · Health**. Compact **Delete Symbol** right-aligned. **Agent Reads** (Assess Symbol / Clear history). Alerts links to filtered **Alerts** / **News**. |
