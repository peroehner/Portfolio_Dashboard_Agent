...
**Fit:** soft ±2 when SAI action aligns with Intent. Soft ±1 when remaining **PT Horizon** (`symbols.target_horizon_at` → `fitExtensions.targetHorizonYearsRemaining`) aligns with Intent (e.g. long remaining ↔ core, short ↔ tactical).  
...
`fitExtensions.holdingPeriodBias` mirrors effective Intent `code`; `fitExtensions.intentOverride` mirrors the user override when set; `fitExtensions.targetHorizonAt` / `targetHorizonYearsRemaining` echo the Pers Target horizon when set.
...
| `targetHorizonAt` | Pers Target horizon end date (implemented) |
| `targetHorizonYearsRemaining` | Years left until `targetHorizonAt` (derived; Fit nudge) |
