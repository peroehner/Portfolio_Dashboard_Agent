...
def technicals_warm_chunk_size() -> int:
    """How many uncached symbols' price history to warm per cycle.

    Warming price history is what lets the Screen / Tech / Sim tabs paint an
    enriched Tech Stance without the on-demand (~40s for a large book) fetch.
    Defaults a bit higher than the fundamentals chunk since it is the main
    driver of Screen-tab latency; still bounded by the process-wide history
    fetch semaphore so it can't burst Yahoo.
    """
    return max(0, int(os.environ.get("TECHNICALS_WARM_CHUNK_SIZE", "6")))


def warm_technicals(queue: list[str], limit: int) -> int:
    """Populate the technical-signals history cache for up to ``limit`` symbols
    in the priority queue that are not already warm. Best-effort per symbol."""
    ...
    for symbol in queue:
        if warmed >= limit:
            break
        if tsvc.history_cached(symbol):
            continue  # already warm — no network needed
        try:
            tsvc.get_chart(symbol)  # fetches + caches the shared price history
            warmed += 1
            logger.info("Technicals warmer: warmed %s (%s/%s)", symbol, warmed, limit)
        except Exception:  # noqa: BLE001 - one symbol must not block the queue
            logger.exception("Technicals warmer failed for %s", symbol)
    return warmed
...
def run_warm_cycle(fundamentals_service: FundamentalsService) -> dict[str, int]:
    ...
    Also pre-warms technical price history (starred → held → rest) so the
    Screen / Tech / Sim tabs paint an enriched Tech Stance without the on-demand
    fetch. Technicals warm even when fundamentals enrichment is disabled, since
    they only need price history.
    """
    queue = build_warm_priority_queue()
    if not queue:
        return {"queued": 0, "warmed": 0, "skipped": 0, "technicalsWarmed": 0}

    # Pre-warm price history first (top-priority symbols), independent of the
    # fundamentals/news warmer below.
    technicals_warmed = warm_technicals(queue, technicals_warm_chunk_size())
    ...
    if not fundamentals_service.enabled:
        return {
            ...
            "warmed": 0,
            "skipped": 0,
            "technicalsWarmed": technicals_warmed,
        }
    ...
    if warmed or technicals_warmed:
        logger.info(
            "Enrichment warm cycle complete: warmed=%s technicals=%s skipped_fresh=%s queue=%s",
            warmed,
            technicals_warmed,
            skipped,
            len(queue),
